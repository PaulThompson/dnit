...
async function createTempFile(content: string, fileName = "test_file.txt"): Promise<string> {
...
Deno.test("UpToDate - timestamp-based change detection", async () => {
  ...
  const timestampBasedHash = (filePath: string, stat: Deno.FileInfo) => {
...
Deno.test("UpToDate - cross-run manifest state consistency", async () => {
  ...
  const ctx1 = createMockExecContext(manifest1);
  ...
  await task1.setup(ctx1);
  await task1.exec(ctx1);
  assertEquals(taskRunCount, 1);
  ...
  const ctx2 = createMockExecContext(manifest2);
  ...
  await task2.setup(ctx2);
  await task2.exec(ctx2);
  ...
  // Third run - should execute because file changed
  await task2.exec(ctx2);
  assertEquals(taskRunCount, 2);
...
Deno.test("UpToDate - custom uptodate with task context access", async () => {
  ...
  const customUptodate = (taskCtx: TaskContext): boolean => {
    ...
    return !!(taskCtx.task.name === "contextTask" && taskCtx.logger && taskCtx.exec);
  };
