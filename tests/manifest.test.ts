...
Deno.test("Manifest - save and load with task data", async () => {
  await withTempDir(async (tempDir) => {
    ...
    assertEquals(loadedManifest.tasks["testTask" as TaskName].lastExecution, "2023-01-01T00:00:00.000Z");
    assertEquals(loadedManifest.tasks["testTask" as TaskName].getFileData("test.txt"), {
      hash: "abc123",
      timestamp: "2023-01-01T00:00:00.000Z"
    });
  });
...
Deno.test("Manifest - multiple save/load cycles preserve data", async () => {
  await withTempDir(async (tempDir) => {
    ...
    const fileData = manifest3.tasks["test" as TaskName].getFileData("file.txt");
