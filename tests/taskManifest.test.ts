...
Deno.test("TaskManifest - getFileData returns undefined for non-existent file", () => {
  ...
  assertEquals(manifest.getFileData("nonexistent.txt" as TrackedFileName), undefined);
});
...
Deno.test("TaskManifest - setFileData adds new file", () => {
  ...
  assertEquals(manifest.getFileData("newfile.txt" as TrackedFileName), fileData);
...
Deno.test("TaskManifest - setFileData updates existing file", () => {
  ...
  assertEquals(manifest.getFileData("existing.txt" as TrackedFileName), newData);
  assertEquals(Object.keys(manifest.trackedFiles).length, 1);
});
...
Deno.test("TaskManifest - toData returns correct structure", () => {
  ...
  assertEquals(exportedData.trackedFiles["file1.txt"], originalData.trackedFiles["file1.txt"]);
  assertEquals(exportedData.trackedFiles["file2.txt"], originalData.trackedFiles["file2.txt"]);
});
...
Deno.test("TaskManifest - round-trip data consistency", () => {
  ...
  assertEquals(manifest2.getFileData("file1.txt" as TrackedFileName), originalData.trackedFiles["file1.txt"]);
});
...
Deno.test("TaskManifest - multiple file operations", () => {
  ...
  assertEquals(manifest.getFileData("file1.txt" as TrackedFileName)?.hash, "hash1");
  assertEquals(manifest.getFileData("file2.txt" as TrackedFileName)?.hash, "updated_hash2");
  assertEquals(manifest.getFileData("file3.txt" as TrackedFileName)?.hash, "hash3");
});
